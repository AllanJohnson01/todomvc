(function() {
  ...
  Bacon.combineTemplate = function(template) {
    ...
    if (typeof template !== "object") return Bacon.constant(template);
    if (template instanceof Observable) return template.toProperty().flatten();
    funcs = [];
    streams = [];
  ...
  Observable = (function() {
    ...
    Observable.prototype.toSettableProperty = function(onPush) {
      var bus, settable;
      if (onPush == null) onPush = nop;
      bus = new Bus();
      settable = addPropertyInitValueToStream(this, bus);
      settable.set = function(value) {
        bus.push(value);
        return onPush(value);
      };
      settable.modify = function(f) {
        return getCurrentValue(settable).map(f).forEach(settable.set);
      };
      settable.end = bus.end;
      return settable;
    };
  ...
  Property = (function(_super) {
    ...
    Property.prototype.flatten = function() {
      return this.flatMapLatest(Bacon.combineTemplate).toProperty();
    };
  ...
  addPropertyInitValueToStream = function(property, stream) {
    return stream.toProperty(getCurrentValue(property));
  };
